...
3.使用 redis 有哪些好处？ 1.速度快，因为数据存在内存中，类似于 HashMap，HashMap 的优势就是查找和操作的时间复杂度都是 O(1)
...
select、poll、epoll。
nginx、redis 都使用了 epoll，这是 linux 平台上的高效 IO 库。
...
11. redis 有几种部署形式，哨兵形式是怎么做的
12. redis 有几种数据结构，它的有序集合是怎么实现的，跳表的结构，跳表如何插入。
13. redis 实现分布式锁
